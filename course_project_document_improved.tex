...
\begin{figure}[h]
  \centering
  \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[node distance=1.6cm and 1.8cm, >=latex]
      \tikzstyle{box}=[draw, rounded corners, align=center, minimum width=3.2cm, minimum height=1.2cm, fill=gray!10, text width=3.3cm]
      \node[box] (xml) {catalog.xml\\(данни + ENTITY)};
      \node[box, right=of xml] (xsd) {catalog.xsd\\(валидация)};
      \node[box, right=of xsd] (xslt) {catalog.xsl\\style.css + JS};
      \node[box, right=of xslt] (html) {HTML таблица\\+ контроли};
      \node[box, right=of html] (browser) {Браузър};
      \draw[->] (xml) -- node[midway, above]{\scriptsize валидация} (xsd);
      \draw[->] (xsd) -- node[midway, above]{\scriptsize XSLT} (xslt);
      \draw[->] (xslt) -- node[midway, above]{\scriptsize трансформация} (html);
      \draw[->] (html) -- node[midway, above]{\scriptsize рендер} (browser);
    \end{tikzpicture}%
  }
